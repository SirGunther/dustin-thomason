...
- **Demonstrated 2026-08-10.** A real upload into Client Deliverables on a live local stack wrote one `outbox_events` row: `type` = `callisto.client-access.file.created.v1`, `aggregate_type` = `File`, and a payload carrying all 16 contract fields — `fileName`, `fileSize` (**unquoted number**, confirming the bigint coercion holds in the real path), `fileType`, `key`, `bucketName`, `length`, `trackTypeId`, `proceedingId`, `attachedToType`, `fileAttachmentId`, `createdUserIdentity`, `createdAt`. The envelope carried `schema.uri` and `schema.version`. **AC1 and AC2 are demonstrated, not merely unit-tested.** Screenshot captured for the PR.
- **Still not demonstrated:** `deliverableCollectionValue` populated for a new and for an existing collection (AC3), and null on a track without collections (AC4). The observed row had null collection fields because no collection was selected — that is correct behavior but does not exercise either criterion. AC4 needs only an Exhibits upload; AC3 is blocked on the `IS_GRANTING_CLIENT_ACCESS_ENABLED` Cognito flag, without which Atlas never shows the collection picker.
