...
### A1 — The ticket's "Why" (living doc) — **BUILT (uncommitted, in review) 2026-07-27**
- **Objective (as refined by the user):** the overarching **"Why" of the whole ticket**, surfaced **early** and kept **living**. Heart = the **class of problem** — establish what we're solving from the start. A two-parter: (1) an initial artifact off the first investigation, and (2) filled in as it goes + how it turned out (the referenced `PRDV-14055-why-these-changes.md` shape — what was missing, bug vs workflow vs other). Logs the reasoning arc per phase: obvious / not obvious / changed after learning more / what got us to the solution / what was noise. If the why moves (problem, class, bug, code, assumption), it's logged, explicitly labeled with the phase.
- **Shipped as:** standalone template `agents/docs/why-these-changes.md`; per-ticket `<slug>-why-these-changes.md` at ticket root; **created Phase 1 (staged in plan, materialized at Phase 2 first action), updated every phase, finalized Phase 6.** Wired: orchestrate standing section "The ticket's Why", folder layout, Phase 1 step 4, Phase 2 materialization, generic phase-gate why-log line, Phase 6 finalize step, README + workflow-index.
- **Boundary (per user: settle only if it bleeds):** distinct from the investigation report (point-in-time class in Phase 2 — this is the running why-thread across all phases, links the report) and from A2/testing-implementation (that's specific scenarios — this is high-level). Leaning multiple artifacts for referenceability ("thinking in pages").
- **Reference-diff enrichment (2026-07-27):** compared the template against the real `PRDV-14055-why-these-changes.md`. Added the finalized spine that was missing — a **categorized change breakdown** (requested change / bug fix / workflow change / capability gap / other, headline count, Before / After / Why per change), **"why it shipped together"** (coherence vs scope creep, tied to acceptance criteria), **Scope**, **Net**, **Verified** (gates + PR link), and an explicit **communication-artifact** purpose. Finalized in Phase 6.
