...
| EV-034 | Gemini's `generateContent` rejects `additionalProperties` inside `generationConfig.responseSchema` with HTTP 400 `INVALID_ARGUMENT` ("Unknown name \"additionalProperties\" at 'generation_config.response_schema': Cannot find field") before authentication. The same schema inside `generationConfig.responseJsonSchema` passes payload validation. An invalid Gemini API key returns HTTP 400 `INVALID_ARGUMENT` whose `error.details[]` holds a `google.rpc.ErrorInfo` with `reason: "API_KEY_INVALID"`, not 401 or 403. | Orchestrator probe during the SAYAI-03 review, 2026-09-22: four `curl` POSTs to `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent` with the placeholder key `fake-probe-key` (no real credential or user content). Cases: `responseSchema` with `additionalProperties` → unknown-field 400; `responseJsonSchema` with `additionalProperties` → `API_KEY_INVALID` 400; `responseSchema` without it → `API_KEY_INVALID` 400; bogus-field control → unknown-field 400 |
| EV-035 | LM Studio's `/v1/chat/completions` applies `reasoning_effort` per request for `google/gemma-4-12b-qat`, overriding the model's saved Enable Thinking setting: `"none"` gave 0 reasoning tokens, while `"minimal"`, `"low"`, `"medium"`, and `"high"` each turned thinking on (335–638 reasoning tokens, 13–25 s) although the saved `enableThinking` was `false`. `reasoning`, `reasoning.effort`, and `chat_template_kwargs.enable_thinking` were ignored. On the LM Studio host, the saved setting produced 321 reasoning tokens of 354 (about 12 s) for SaySlate's request. | Local probes against `http://127.0.0.1:1234/v1` on 2026-09-23, one request per value; `%USERPROFILE%\.lmstudio\.internal\user-concrete-model-default-config\google\gemma-4-12b-qat.json` — `enableThinking: false`; LM Studio host log supplied by Dustin Thomason, 2026-09-23 |
