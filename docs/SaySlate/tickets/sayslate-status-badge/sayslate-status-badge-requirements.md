...
| REQ-004 | On the full page, a keyboard shortcut does not act while an AI pass runs if its button is disabled for that pass. | Dustin Thomason, 2026-09-24, directing the work after the orchestrating agent asked whether to block Ctrl+Alt+D and Ctrl+Alt+X during a pass: "It doesn't matter to me, that's not really something i need to decide on" … "Do the work now." |
...
| EV-027 | At `753a5b1`, the Ctrl+Alt+R shortcut copies the processed result while the second pass runs, although the copy-result button is disabled then. The shortcut handler checks only the Finish workflow before it dispatches any key. | `C:\SaySlate\app.js:747` — `copyResultButton.disabled = words === 0 \|\| secondPassRunning \|\| finishWorkflowRunning;`; `app.js:1420-1448` (`handleShortcut`: `if (finishWorkflowRunning) {` … `} else if (key === "r") {` / `void copyResultTranscript();`) |
| EV-028 | At `753a5b1`, the Finish workflow refuses to start while either pass runs and says so with the toast "AI processing is already running". | `C:\SaySlate\app.js:1370-1373` — `if (firstPassRunning \|\| secondPassRunning) {` / `showToast("AI processing is already running");` / `return;` |
| EV-029 | At `753a5b1`, the full page starts dictation only through `toggleDictation`, and a pass never starts while dictating. So once Ctrl+Alt+D is blocked during a pass, dictation and a pass cannot overlap. | `C:\SaySlate\app.js:1409-1418` (`toggleDictation`, the only caller of `startDictation` and `startLocalWhisper`); `app.js:1452` (start button) and `app.js:1436` (Ctrl+Alt+D) are its only callers; `setListeningUI(true)` runs only from a session those functions started (`app.js:1025`, `1261`), and `handleLocalWhisperEvent` ignores other sessions (`app.js:1204`); `app.js:807` and `app.js:870` — the pass guards `… \|\| isListening) return false;` |
