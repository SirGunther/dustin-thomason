...
| LD-007 | **Superseded by LD-009.** When dictation and a pass overlap (EV-026), the dictation state owns the badge while dictation runs: a pass that succeeds or fails during dictation does not replace "Listening". When dictation stops while a pass request is still in flight, the badge shows that pass's `processing` / "Phase N" instead of "Ready". | The badge reports the current activity ([origin](./sayslate-status-badge-original-ticket.md), message 1, "displays the current status of the process"). While dictating, that is "Listening" (EV-003), which LD-005 already keeps over discard and clear. Once dictation stops, the running pass is the current activity, and "Ready" would report a stage that is still running, against REQ-001. | REQ-001 | REQ-001; EV-003; LD-005 state 4's "Listening" rule; resolved from sources, open decision 4 | Narrows LD-005 states 1–3 |
| LD-008 | On the full page, Ctrl+Alt+D and Ctrl+Alt+X do nothing while either pass runs, and Ctrl+Alt+R does nothing while the second pass runs. Each shows the toast "AI processing is already running" instead. | Each shortcut is documented as its button's action (`C:\SaySlate\README.md:11-16`), and each button is disabled under exactly these conditions (EV-025, EV-026, EV-027). The toast is the wording Finish already uses for the same refusal (EV-028), so no new wording is added. Ctrl+Alt+R has the same gap as the two shortcuts named in the question, so the rule covers all three. | REQ-004 | REQ-004; EV-025–EV-028; resolved from sources, open decision 5 | — |
| LD-009 | SAYSTAT-01's LD-007 overlap handling is removed. The pass result states are set without a dictation check, and stopping dictation returns to "Ready" as it did before SAYSTAT-01. `activePassLabel` stays, for LD-006 only. | With LD-008 in place, dictation and a pass cannot overlap (EV-029), so those branches can no longer run and cannot be tested. They compensated for the shortcut gap that LD-008 closes at its source. The discard button is still enabled during the first pass (EV-025), so LD-006 remains reachable. | REQ-001 | LD-008; EV-029 | Supersedes LD-007 |
...
- [x] 4. What does the full-page badge show when dictation starts or stops while a pass request is in flight? — Resolved from sources as LD-007, later superseded by LD-009
- [x] 5. Should Ctrl+Alt+D and Ctrl+Alt+X act while a pass runs, although their buttons are disabled then? — Resolved from sources as LD-008
...
### 5. Full-page shortcuts while a pass runs

**State:** Resolved
**Value:** Resolved from sources as LD-008, with LD-009 removing LD-007's overlap handling.
**Investigation:** SAYSTAT-01's audit recorded as residual risk that Ctrl+Alt+D and Ctrl+Alt+X act during a pass while their buttons are disabled (EV-025, EV-026). The orchestrating agent wrongly handed this to the user as a decision. On investigation, Ctrl+Alt+R has the same gap during the second pass (EV-027). Finish already refuses during a pass with a toast (EV-028). Blocking Ctrl+Alt+D removes every path by which dictation and a pass can overlap (EV-029).
**Why user input is required:** Not required. Each button's disabled condition already encodes the intended rule for its shortcut. Dustin Thomason, 2026-09-24: "that's not really something i need to decide on" … "Do the work now." (REQ-004)
**Recommendation:** —
**Evidence:** EV-025, EV-026, EV-027, EV-028, EV-029; REQ-004
**Depends on:** —
