...
| Phase | Name | Mode/Reasoning | Output artifacts | Advance |
| --- | --- | --- | --- | --- |
| 0 | Capture | Working / Low | `original-ticket.md`, `orchestration.md`, job stories (draft) + index | gate → HANDOFF (Plan) |
| 1 | Recon and plan | Plan / Extra High | approved recon-and-plan doc (findings + emission todos + staged coverage rows) | gate → plan approval |
| 2 | Report | Working / High | investigation report, coverage ledger, diagrams, test-plan seed | gate → AUTO-ADVANCE to 3 |
| 3 | Probe & spec | Working / Extra High | locked-decision ledger, accepted job stories, spec, spec submitted to its reviewer, refined test plan | gate → HANDOFF (Plan) |
| 4 | Prep | Plan / Extra High | approved implementation plan | gate → plan approval |
| 5 | Implement | Working / High | **reviewer's spec response recorded**, code, executed test plan, session log, PR | gate → HANDOFF (Idle) |
| 6 | Wrap up | Working / Low | finalized why doc, closed stories, closed ledger, review summary | END — you review manually after |
