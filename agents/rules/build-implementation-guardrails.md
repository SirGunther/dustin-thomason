...
- The **testing-implementation artifact** (`testing/<slug>-testing-implementation.md`, template `testing-implementation-artifact.md`) is where this content is assembled while testing — **scenario-first**: the real situations stress-tested (why each matters, newly-uncovered ones flagged), with each change hung off the scenario that forced it (file + observed → expected → fix). It is the staging ground for the PR comment; it is **never** copied into the source as a code comment.
