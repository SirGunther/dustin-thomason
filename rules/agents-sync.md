...
All three are committed and machine-neutral. `scripts/sync-rules.ps1` regenerates them; direct edits to any output are overwritten on the next run.

When you **create, edit, rename, or delete** any file under `rules/` (or `.cursor/skills/`):

1. Run **`.\scripts\sync-rules.ps1`** from the repo root before you finish.
...
```
Good: edit rules/<rule>.md, then run .\scripts\sync-rules.ps1, then stage the outputs.
